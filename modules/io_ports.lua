...
local function set_port_range(self, start_port, end_port, write_func, read_func)
    for i = start_port, end_port, 1 do
        self.in_ports[i] = read_func
        self.out_ports[i] = write_func
    end
end
...
local function in_port(self, port)
    ...
    if func then
        return func(self.in_args[port], self.cpu, port)
    end
...
local function out_port(self, port, val)
    ...
    if func then
        func(self.out_args[port], self.cpu, port, val)
        return
    end
end

local function set_in_function_argument(self, port, arg)
    self.in_args[port] = arg
end

local function set_out_function_argument(self, port, arg)
    self.out_args[port] = arg
end

local function set_function_argument(self, port, arg)
    self.in_args[port] = arg
    self.out_args[port] = arg
end

local function set_function_argument_range(self, start_port, end_port, arg)
    for i = start_port, end_port, 1 do
        self.in_args[i] = arg
        self.out_args[i] = arg
    end
...
function io_ports.new(cpu)
    local self = {
        ...
        set_port_range = set_port_range,
        in_port = in_port,
        out_port = out_port,
        get_port = get_port,
        set_port = set_port,
        set_in_function_argument = set_in_function_argument,
        set_out_function_argument = set_out_function_argument,
        set_function_argument = set_function_argument,
        set_function_argument_range = set_function_argument_range
    }
