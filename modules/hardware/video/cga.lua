-- =====================================================================================================================================================================
-- IBM Color Graphics Adapter (CGA) emulation.
-- =====================================================================================================================================================================

local common = require("emulator:hardware/video/common")
local band, bor, rshift, lshift, bxor, bnot = bit.band, bit.bor, bit.rshift, bit.lshift, bit.bxor, bit.bnot

local videocard = {}

local palette = {
    [0] = 0x000000, -- Black
    0x0000AA, -- Blue
    0x00AA00, -- Green
    0x00AAAA, -- Cyan
    0xAA0000, -- Red
    0xAA00AA, -- Magenta
    0xAA5500, -- Brown
    0xAAAAAA, -- Light gray
    0x555555, -- Dark gray
    0x5555FF, -- Light blue
    0x55FF55, -- Light green
    0x55FFFF, -- Light cyan
    0xFF5555, -- Light red
    0xFF55FF, -- Light magenta
    0xFFFF55, -- Yellow
    0xFFFFFF  -- White
}

local palettes = {
    {[0] = 0, 2, 4, 6},
    {[0] = 0, 10, 12, 14},
    {[0] = 0, 3, 5, 7},
    {[0] = 0, 11, 13, 15},
    {[0] = 0, 3, 4, 7},
    {[0] = 0, 11, 12, 15}
}

local font_8_8 = {
	[0] = 
}

local wait_states = {[0] = 3, 4, 5, 6, 7, 8, 4, 5, 6, 7, 8, 4, 5, 6, 7, 8}

local function update_timings(self)
    local disp_time = self.crtc_horizontal_total + 1
    local disp_on_time = self.crtc_horizontal_displayed

    if not self.mode_hires_text then
        disp_time = lshift(disp_time, 1)
        disp_on_time = lshift(disp_on_time, 1)
    end

    self.disp_off_time = math.floor((disp_time - disp_on_time) * self.clock)
    self.disp_on_time = math.floor(disp_on_time * self.clock)
end

local crtc_regs_write = {
    [0x00] = function(self, val) -- Horizontal Total Register
        self.crtc_horizontal_total = val
        update_timings(self)
    end,
    [0x01] = function(self, val) -- Horizontal Displayed Register
        self.crtc_horizontal_displayed = val
        self.width = lshift(self.crtc_horizontal_displayed, 3)

        if self.width < 640 then
            self.screen:set_scale(2.0, 2.0)
        else
            self.screen:set_scale(1.0, 2.0)
        end

        self.screen:set_resolution(math.clamp(self.width, 320, 640), 200)

        update_timings(self)
    end,
    [0x02] = function(self, val) -- Horizontal Sync Position Register
    end,
    [0x03] = function(self, val) -- Horizontal Sync Pulse Width Register
    end,
    [0x04] = function(self, val) -- Vertical Total Register
        self.crtc_vertical_total = band(val, 0x7F)
    end,
    [0x05] = function(self, val) -- Vertical Total Adjust Register
    end,
    [0x06] = function(self, val) -- Vertical Displayed Register
        self.crtc_vertical_displayed = band(val, 0x7F)
    end,
    [0x07] = function(self, val) -- Vertical Sync Register
        self.crtc_vsync = band(val, 0x7F)
    end,
    [0x08] = function(self, val) -- Interlase Mode Register
    end,
    [0x09] = function(self, val) -- Max Scan Line Register
        self.crtc_max_scanline = band(val, 0x1F)
    end,
    [0x0A] = function(self, val) -- Cursor Start Register
        self.crtc_cursor_start = band(val, 0x1F)
        self.cursor_shape = band(val, 0x60)
    end,
    [0x0B] = function(self, val) -- Cursor End Register
        self.crtc_cursor_end = band(val, 0x1F)
    end,
    [0x0C] = function(self, val) -- Start Address Register High
        self.crtc_start_addr = bor(band(self.crtc_start_addr, 0x00FF), lshift(band(val, 0x3F), 8))
    end,
    [0x0D] = function(self, val) -- Start Address Register Low
        self.crtc_start_addr = bor(band(self.crtc_start_addr, 0xFF00), val)
    end,
    [0x0E] = function(self, val) -- Cursor Location Register High
        self.crtc_cursor_addr = bor(band(self.crtc_cursor_addr, 0x00FF), lshift(val, 8))
    end,
    [0x0F] = function(self, val) -- Cursor Location Register Low
        self.crtc_cursor_addr = bor(band(self.crtc_cursor_addr, 0xFF00), val)
    end
}

local crtc_regs_read = {
    [0x0E] = function(self)
        return band(rshift(self.crtc_cursor_addr, 8), 0xFF)
    end,
    [0x0F] = function(self)
        return band(self.crtc_cursor_addr, 0xFF)
    end,
    [0x10] = function(self)
        return band(rshift(self.crtc_lp_latch, 8), 0xFF)
    end,
    [0x11] = function(self)
        return band(self.crtc_lp_latch, 0xFF)
    end
}

local function update_palette(self)
    local index

    if self.mode_bw and self.mode_graphics and (not self.mode_hires_graphics) then
        index = 5
    elseif band(self.cc_register, 0x20) ~= 0 then
        index = 3
    else
        index = 1
    end

    if band(self.cc_register, 0x10) ~= 0 then
        index = index + 1
    end

    self.cc_altcolor = band(self.cc_register, 0x0F)
    self.palette = palettes[index]
    self.palette_mono[1] = palette[self.cc_altcolor]
end

local function update_latch(self)
    self.crtc_lp_latch = self.current_line * self.crtc_horizontal_displayed
end

------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- VRAM
------------------------------------------------------------------------------------------------------------------------------------------------------------------------

local function vram_read(self, addr)
    self.cpu.cycles = self.cpu.cycles - wait_states[band(self.cpu.cycles, 0x0F)]
	return self.vram[band(addr, 0x3FFF)]
end

local function vram_write(self, addr, val)
    self.cpu.cycles = self.cpu.cycles - wait_states[band(self.cpu.cycles, 0x0F)]
	self.vram[band(addr, 0x3FFF)] = val
end

------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Render
------------------------------------------------------------------------------------------------------------------------------------------------------------------------

local function render_line_empty(self)
    local screen_addr = self.current_line * self.width

    for x = 0, self.width - 1, 1 do
        self.screen:set_pixel_rgb_i(screen_addr + x, 0x000000)
    end
end

local function render_line_text(self)
    local screen_addr = self.current_line * self.width
    local draw_cursor, attr, background_index, foreground_index, background, foreground, glyph_row, color

    for x = 0, self.crtc_horizontal_displayed - 1, 1 do
        draw_cursor = (self.vma == self.crtc_cursor_addr) and self.cursor_visible and self.cursor_on
        glyph_row = font_8_8[lshift(self.vram[band(lshift(self.vma, 1), 0x3FFF)], 3) + band(self.scanline, 0x07)]
        attr = self.vram[band(lshift(self.vma, 1) + 1, 0x3FFF)]

        if self.mode_blink then
            background_index = band(rshift(attr, 4), 0x07)

            if (band(self.blink, 0x08) ~= 0) and (band(attr, 0x80) ~= 0) and (not draw_cursor) then
                foreground_index = background_index
            else
                foreground_index = band(attr, 0x0F)
            end
        else
            background_index = band(rshift(attr, 4), 0x0F)
            foreground_index = band(attr, 0x0F)
        end

        if draw_cursor then
            background = palette[bxor(background_index, 0x0F)]
            foreground = palette[bxor(foreground_index, 0x0F)]
        else
            background = palette[background_index]
            foreground = palette[foreground_index]
        end

        for i = 0, 7, 1 do
            if band(glyph_row, rshift(0x80, i)) ~= 0 then
                color = foreground
            else
                color = background
            end

            self.screen:set_pixel_rgb_i(screen_addr + lshift(x, 3) + i, color)
        end

        self.vma = self.vma + 1
    end
end

local function render_line_graphics_low(self)
    local y, addr, data, index = self.current_line * 320, nil, nil, nil

    for x = 0, 319, 8 do
        addr = band(lshift(self.vma, 1), 0x1FFF) + (band(self.scanline, 0x01) * 0x2000)
        data = bor(lshift(self.vram[addr], 8), self.vram[addr + 1])

        self.vma = self.vma + 1

        for i = 0, 7, 1 do
            index = band(rshift(data, 14), 0x03)

            if index == 0 then
                index = self.cc_altcolor
            else
                index = self.palette[index]
            end

            self.screen:set_pixel_rgb_i(y + x + i, palette[index])
            data = lshift(data, 2)
        end
    end
end

local function render_line_graphics_high(self)
    local y = self.current_line * 640

    for x = 0, 639, 8 do
        local addr = band(self.vma, 0x1FFF) + (band(self.scanline, 0x01) * 0x2000)
        local data = self.vram[addr]

        self.vma = self.vma + 1

        for i = 0, 7, 1 do
            self.screen:set_pixel_rgb_i(y + x + i, self.palette_mono[band(rshift(data, 7), 0x01)])
            data = lshift(data, 1)
        end
    end
end

------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Ports
------------------------------------------------------------------------------------------------------------------------------------------------------------------------

local function port_crtc_register_select_out(self, cpu, port, val)
    self.crtc_index = band(val, 0x1F)
end

local function port_crtc_register_select_in(self, cpu, port)
    return self.crtc_index
end

local function port_crtc_register_out(self, cpu, port, val)
    local reg = crtc_regs_write[self.crtc_index]

    if reg then
        reg(self, val)
    end
end

local function port_crtc_register_in(self, cpu, port)
    local reg = crtc_regs_read[self.crtc_index]

    if reg then
        return reg(self)
    end

    return 0xFF
end

local function port_mode_register(self, cpu, port, val)
    local mode_changed = (self.mode_hires_text ~= (band(val, 0x01) ~= 0)) or (self.mode_graphics ~= (band(val, 0x02) ~= 0)) or (self.mode_hires_graphics ~= (band(val, 0x10) ~= 0))

    self.mode_hires_text = band(val, 0x01) ~= 0
    self.mode_graphics = band(val, 0x02) ~= 0
    self.mode_enable = band(val, 0x08) ~= 0
    self.mode_hires_graphics = band(val, 0x10) ~= 0
    self.mode_blink = band(val, 0x20) ~= 0
    self.mode_bw = band(val, 0x04) ~= 0

    if mode_changed then
        if self.mode_graphics then
            if self.mode_hires_graphics then
                self.screen:set_scale(1.0, 2.0)
                self.width = 640
                self.render_line = render_line_graphics_high
            else
                self.screen:set_scale(2.0, 2.0)
                self.width = 320
                self.render_line = render_line_graphics_low
            end

            self.screen:set_resolution(self.width, 200)
        else
            self.width = lshift(self.crtc_horizontal_displayed, 3)

            if self.mode_hires_text then
                self.screen:set_scale(1.0, 2.0)
            else
                if self.width < 640 then
                    self.screen:set_scale(2.0, 2.0)
                end
            end

            self.render_line = render_line_text
            self.screen:set_resolution(math.clamp(self.width, 320, 640), 200)
        end
    end

    update_palette(self)
    update_timings(self)
end

local function port_color_control_register(self, cpu, port, val)
    self.cc_register = val
    update_palette(self)
end

local function port_status_regsiter_in(self, cpu, port)
    return self.status
end

local function port_lp_reset_register_out_in(self, cpu, port, val)
    if self.lp_latch then
        self.lp_latch = false
        self.status = bor(band(self.status, bnot(0x02)), 0x04)
    end

    if not val then
        return 0xFF
    end
end

local function port_lp_set_register_out_in(self, cpu, port, val)
    if not self.lp_latch then
        self.lp_latch = true
        self.status = band(bor(self.status, 0x02), bnot(0x04))
        update_latch(self)
    end

    if not val then
        return 0xFF
    end
end

------------------------------------------------------------------------------------------------------------------------------------------------------------------------

local update_2

local function update_1(self)
    self.timer:advance(self.disp_off_time)
    self.timer.callback = update_2

    self.status = bor(self.status, 0x01)

    if self.display_on and self.mode_enable then
        self.render_line(self)
    elseif self.display_on then
        render_line_empty(self)
    end

    self.current_line = self.current_line + 1

    if self.current_line >= 360 then
        self.current_line = 0
    end

    if (self.vertical_count == self.crtc_vsync) and (self.scanline == 0) then
        self.status = bor(self.status, 0x08)
    end
end

update_2 = function(self)
    local last_cursor_state = self.cursor_visible

    self.timer:advance(self.disp_on_time)
    self.timer.callback = update_1

    if self.vsync_time > 0 then
        self.vsync_time = self.vsync_time - 1

        if self.vsync_time == 0 then
            self.status = band(self.status, bnot(0x08))
        end
    end

    if self.scanline == self.crtc_cursor_end then
        self.cursor_visible = false
    end

    if self.scanline == self.crtc_max_scanline then
        local old_vc = self.vertical_count

        self.scanline = 0
        self.vma_t = self.vma
        self.vertical_count = band(self.vertical_count + 1, 0x7F)

        if self.vertical_count == self.crtc_vertical_displayed then
            self.display_on = false
        end

        if old_vc == self.crtc_vertical_total then
            self.vertical_count = 0
            self.vma = self.crtc_start_addr
            self.vma_t = self.vma
            self.display_on = true

            if self.cursor_shape == 0x20 then
                self.cursor_on = false
            elseif self.cursor_shape == 0x60 then
                self.cursor_on = band(self.blink, 0x10) ~= 0
            else
                self.cursor_on = band(self.blink, 0x08) ~= 0
            end

            self.current_line = 0
        end

        if self.vertical_count == self.crtc_vsync then
            self.display_on = false
            self.current_line = 0
            self.blink = band(self.blink + 1, 0x7F)
            self.vsync_time = 16

            if self.crtc_vsync > 0 then
                self.screen:update()
            end
        end
    else
        self.scanline = band(self.scanline + 1, 0x1F)
        self.vma = self.vma_t
    end

    if self.display_on then
        self.status = band(self.status, bnot(0x01))
    end

    if self.scanline == self.crtc_cursor_start then
        self.cursor_visible = true
    end
end

local function reset(self)
    self.crtc_index = 0
    self.status = 0xF0
    self.palette = palettes[1]
    self.current_line = 0
    self.vma = 0
    self.vma_t = 0
    self.blink = 0
    self.scanline = 0
    self.vertical_count = 0
    self.width = 0
    self.cursor_shape = 0
    self.crtc_horizontal_displayed = 80
    self.crtc_vertical_total = 0
    self.crtc_vertical_displayed = 0
    self.crtc_vsync = 0
    self.crtc_max_scanline = 0
    self.crtc_cursor_start = 0
    self.crtc_cursor_end = 0
    self.crtc_start_addr = 0
    self.crtc_cursor_addr = 0
    self.crtc_lp_latch = 0
    self.cc_altcolor = 0
    self.cc_register = 0
    self.vsync_time = 0
    self.display_on = false
    self.cursor_visible = false
    self.mode_enable = false
    self.mode_graphics = false
    self.mode_hires_graphics = false
    self.mode_hires_text = false
    self.mode_blink = false
    self.mode_bw = false
    self.lp_latch = false
    self.palette_mono[1] = palette[0x0F]
    self.timer.callback = update_1
    self.timer:set_delay(self.disp_off_time)
    self.screen:set_scale(1.0, 2.0)
    self.screen:set_resolution(640, 200)
end

local function set_clock(self, clock)
    self.clock = clock
    update_timings(self)
end

local function get_type(self)
    return common.TYPE.CGA
end

function videocard.new(cpu, memory, screen)
    local self = {
        cpu = cpu,
        screen = screen,
        crtc_index = 0,
        status = 0xF0,
        palette = palettes[1],
        palette_mono = {[0] = palette[0x00], palette[0x0F]},
        current_line = 0,
        vma = 0, -- Video Memory Address
        vma_t = 0,
        blink = 0,
        scanline = 0,
        vertical_count = 0,
        width = 0,
        cursor_shape = 0x00,
        crtc_horizontal_total = 0,
        crtc_horizontal_displayed = 0,
        crtc_vertical_total = 0,
        crtc_vertical_displayed = 0,
        crtc_vsync = 0,
        crtc_max_scanline = 0,
        crtc_cursor_start = 0,
        crtc_cursor_end = 0,
        crtc_start_addr = 0,
        crtc_cursor_addr = 0,
        crtc_lp_latch = 0,
        cc_altcolor = 0,
        cc_register = 0,
        disp_off_time = 320,
        disp_on_time = 320,
        vsync_time = 0,
        display_on = false,
        cursor_visible = false,
        mode_enable = false,
        mode_graphics = false,
        mode_hires_graphics = false,
        mode_hires_text = false,
        mode_blink = false,
        mode_bw = false,
        lp_latch = false,
        vram = {},
        render_line = render_line_text,
        vram_read = vram_read,
        vram_write = vram_write,
        set_clock = set_clock,
        get_type = get_type,
        reset = reset
    }

    local cpu_io = cpu:get_io()

    cpu_io:set_port(0x3D4, port_crtc_register_select_out, port_crtc_register_select_in)
    cpu_io:set_port(0x3D5, port_crtc_register_out, port_crtc_register_in)
    cpu_io:set_port_out(0x3D8, port_mode_register)
    cpu_io:set_port_out(0x3D9, port_color_control_register)
    cpu_io:set_port_in(0x3DA, port_status_regsiter_in)
    cpu_io:set_port(0x3DB, port_lp_reset_register_out_in, port_lp_reset_register_out_in)
    cpu_io:set_port(0x3DC, port_lp_set_register_out_in, port_lp_set_register_out_in)

    cpu_io:set_function_argument_range(0x3D4, 0x3D5, self)
    cpu_io:set_out_function_argument(0x3D8, self)
    cpu_io:set_out_function_argument(0x3D9, self)
    cpu_io:set_in_function_argument(0x3DA, self)
    cpu_io:set_function_argument_range(0x3DB, 0x3DC, self)

    -- Initialize VRAM
    for i = 0, 0x3FFF, 1 do
        self.vram[i] = 0x00
    end

    memory:add_mapping(0xB8000, 0x8000, vram_read, vram_write, self)

    -- Initialize timer
    self.timer = cpu:get_scheduler():add(update_1, self, false)

    return self
end

return videocard
