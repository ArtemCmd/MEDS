...
local common = require("emulator:hardware/video/common")
...
local EGA_ROM = file.read_bytes("emulator:roms/video/ibm_6277356_ega_card_u44_27128.bin")
...
local gc_modes = {
    [0x00] = function(self, vram_addr, write_mask, val)
        local data = ega_rotate[lshift(self.gc_data_rotate_count, 8) + val]
...
local function update_timings(self)
    ...
    if band(self.mor, 0x04) ~= 0 then
        clock = self.cpu:get_clock() / 16257000.0
    else
        clock = self.cpu:get_clock() / (157500000.0 / 11.0)
    end

    clock = math.floor(clock * (8 + (self.seq_dot_clocks and 0 or 1)))

    if self.seq_dot_clock then
        self.delay_on = lshift(self.crtc_horizontal_display_end, 1)
        self.delay_off = (lshift(self.crtc_horizontal_total + 2, 1) - self.delay_on) * clock
    else
        self.delay_on = self.crtc_horizontal_display_end
        self.delay_off = (self.crtc_horizontal_total + 2 - self.delay_on) * clock
    end

    self.delay_on = self.delay_on * clock
...
local function get_addr(self, addr)
    if self.crtc_word_mode then -- Word Mode
        ...
        addr = bor(lshift(addr, 1), bit)
    end

    if self.crtc_cga_compatibility_mode then
        addr = bor(band(addr, bnot(0x8000)), lshift(band(self.scanline, 0x01), 15))
    end
...
local function render_text(self)
    ...
    for x = 0, self.width - 1, self.glyph_width do
        local addr = get_addr(self, self.vma)
        ...
        if (self.vma == lshift(self.crtc_cursor_pos, 2)) and self.cursor_visible and self.cursor_on then
...
local function render_graphics(self)
    ...
    for x = 0, self.width - 1, self.glyph_width do
        ...
        if self.seq_shift_load then
            data_1 = self.vram[bxor(addr, clock)]
            data_2 = self.vram[bxor(bor(addr, 0x01), clock)]
            data_3 = self.vram[bxor(bor(addr, 0x02), clock)]
            data_4 = self.vram[bxor(bor(addr, 0x03), clock)]
            ...
            data_1 = self.vram[addr]
            data_2 = self.vram[bor(addr, 0x01)]
            data_3 = self.vram[bor(addr, 0x02)]
            data_4 = self.vram[bor(addr, 0x03)]

            self.vma = self.vma + 4
        end

        self.vma = band(self.vma, 0x3FFFF)

        if self.gc_shift_mode then
            local temp1 = bor(cga2ega[data_2], lshift(cga2ega[data_1], 4))
            local temp2 = bor(cga2ega[rshift(data_2, 1)], lshift(cga2ega[rshift(data_1, 1)], 4))
            local temp3 = bor(cga2ega[data_4], lshift(cga2ega[data_3], 4))
            local temp4 = bor(cga2ega[rshift(data_4, 1)], lshift(cga2ega[rshift(data_3, 1)], 4))
        ...
        for i = 0, 7, 2 do
            local in_shift = 6 - i
            local pixel_data = bor(pixel_data_lookup[lshift(band(rshift(data_1, in_shift), 0x03), 2) + band(rshift(data_2, in_shift), 0x03)], lshift(pixel_data_lookup[lshift(band(rshift(data_3, in_shift), 0x03), 2) + band(rshift(data_4, in_shift), 0x03)], 2))
...
local function port_mor_out(self, cpu, port, val) -- Miscellaneous Output Register
    ...
    if band(self.mor, 0x01) ~= io_address then
        ...
        if io_address == 0x01 then
            ...
            cpu_io:set_function_argument(0x3B4, nil)
            cpu_io:set_function_argument(0x3B5, nil)
            cpu_io:set_function_argument(0x3BA, nil)
        else
            ...
            cpu_io:set_function_argument(0x3B4, self)
            cpu_io:set_function_argument(0x3B5, self)
            cpu_io:set_function_argument(0x3BA, self)
        end
    end
...
local function port_isr0_in(self, cpu, port) -- Input Status Register 0 
    return bor(lshift(band(rshift(self.switch, 3 - self.switch_select), 0x01), 4), lshift(self.fcr, 5))
end

local function port_fcr_out(self, cpu, port, val)
    self.fcr = val
end

local function port_isr1_in(self, cpu, port) -- Input Status Register 1
    self.ac_flipflop = false
    self.status1 = bxor(self.status1, 0x30)
    return self.status1
end
...
local function port_seq_addr_register_out(self, cpu, port, val)
    self.seq_index = band(val, 0x0F)
end

local function port_seq_data_register_out(self, cpu, port, val)
...
local crtc_regs_write = {
    ...
    [0x0C] = function(self, val) -- Start Address Register HIGH
        self.vma_latch = bor(band(self.vma_latch, 0x00FF), lshift(val, 8))
    end,
    [0x0D] = function(self, val) -- Start address Register LOW
        self.vma_latch = bor(band(self.vma_latch, 0xFF00), val)
    end,
    ...
    [0x11] = function(self, val) -- Vertical Retrace End Register
        self.crtc_vertical_retrace_end = band(val, 0x0F)
    end,
...
local function port_crtc_addr_register_out(self, cpu, port, val)
    self.crtc_index = band(val, 0x1F)
end

local function port_crtc_addr_register_in(self, cpu, port)
    return self.crtc_index
end

local function port_crtc_data_register_out(self, cpu, port, val)
...
local function port_crtc_data_register_in(self, cpu, port)
...
local gc_regs = {
    ...
    [0x06] = function(self, val) -- Miscellaneous Register
        ...
        self.memory:add_mapping(self.vram_start, vram_size, vram_read, vram_write, self)
...
local function port_gc_addr_out(self, cpu, port, val)
    self.gc_index = band(val, 0x0F)
end

local function port_gc_data_out(self, cpu, port, val)
...
local function port_ac_out(self, cpu, port, val)
...
local function update_1(self)
    ...
    self.status1 = bor(self.status1, 0x01)
    ...
    if (band(self.status1, 0x08) ~= 0) and (band(self.current_line, 0x0F) == self.crtc_vertical_retrace_end) and (self.vertical_line > 0) then
        self.status1 = band(self.status1, bnot(0x08))
    end

    self.vertical_line = self.vertical_line + 1
end
...
update_2 = function(self)
    ...
    if self.display_on then
        self.status1 = band(self.status1, bnot(0x01))
    end
    ...
    if self.display_on then
        if self.scanline == self.crtc_max_scanline then
            self.scanline = 0
            self.vma_t = band(self.vma_t + lshift(self.crtc_offset, 3), self.memory_mask)
            self.vma = self.vma_t
        else
    ...
    if self.vertical_count == self.crtc_vertical_retrace_start then
        self.display_on = false
        self.vma_t = lshift(self.vma_latch, 2)
        self.vma = self.vma_t
        self.vertical_line = 0
        self.status1 = bor(self.status1, 0x08)
    end
...
local function reset(self)
    ...
    self.crtc_vertical_retrace_start = 350
    self.crtc_vertical_retrace_end = 350
...
function ega.new(cpu, memory, screen)
    local self = {
        ...
        current_line = 0,
        vertical_count = 0,
        vma = 0x00000, -- Video Memory Address
        vma_t = 0x00000,
        vma_latch = 0x00000,
        memory_mask = 0xFFFF,
        scanline = 0,
    ...
    cpu_io:set_port_out_range(0x3C0, 0x3C1, port_ac_out)
    cpu_io:set_port(0x3C2, port_mor_out, port_isr0_in)
    cpu_io:set_port(0x3DA, port_fcr_out, port_isr1_in)
    cpu_io:set_port_out(0x3C4, port_seq_addr_register_out)
    cpu_io:set_port_out(0x3C5, port_seq_data_register_out)
    cpu_io:set_port(0x3D4, port_crtc_addr_register_out, port_crtc_addr_register_in)
    cpu_io:set_port(0x3D5, port_crtc_data_register_out, port_crtc_data_register_in)
    cpu_io:set_port_out(0x3CE, port_gc_addr_out)
    cpu_io:set_port_out(0x3CF, port_gc_data_out)

    cpu_io:set_out_function_argument(0x3C0, self)
    cpu_io:set_out_function_argument(0x3C1, self)
    cpu_io:set_function_argument(0x3C2, self)
    cpu_io:set_function_argument(0x3DA, self)
    cpu_io:set_out_function_argument(0x3C4, self)
    cpu_io:set_out_function_argument(0x3C5, self)
    cpu_io:set_function_argument(0x3D4, self)
    cpu_io:set_function_argument(0x3D5, self)
    cpu_io:set_out_function_argument(0x3CE, self)
    cpu_io:set_out_function_argument(0x3CF, self)
    ...
    memory:add_mapping(0xC0000, 0x08000, rom_read, rom_write, self.rom)
    memory:add_mapping(0xA0000, 0x20000, vram_read, vram_write, self)
