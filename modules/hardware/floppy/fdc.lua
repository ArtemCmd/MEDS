...
local function create_drive(self, drive_num)
...
local function command_seek(self)
    ...
    self.drives[drive_id].cylinder = cylinder
    self.drive_select = drive_id
    self.msr = bor(FDC_STATUS_MRQ, lshift(1, self.drive_select))
    self.last_error = ST1_NOERROR
    self.operation = OPERATION_SEEK
...
local operations = {
    [OPERATION_SEEK] = function(self) -- Recalibrate / Seek
        self.pic:request_interrupt(FDC_IRQ)
        self.pending_interrupt = true
    end,
    [OPERATION_READ] = function(self) -- Read Data
        local drive = self.drives[self.drive_select]
        ...
        for i = 1, sector_size, 1 do
            if self.dma:channel_write(FDC_DMA, self.sector_buffer[i], false) == 0x100 then
                if self.sector == self.sectors then
                    if not self.mt then
                        self.cylinder = self.cylinder + 1
                        self.sector = 1
    ...
    [OPERATION_WRITE] = function(self) -- Write Data
        local drive = self.drives[self.drive_select]
        local sector_size = sector_sizes[self.sector_size]
        ...
        for i = 1, sector_size, 1 do
            ...
            self.sector_buffer[i] = band(result, 0xFF)
...
local function insert_drive(self, num, path, write_protected)
    ...
    if not success then
        logger:error("FDC: Failed to load floppy file: \"%s\"", result)
    end
...
local function set_clock(self)
    self.delay = 10000 * self.timer.scheduler.NANOSECOND
