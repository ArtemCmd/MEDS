...
local function read_sector(self, cylinder, head, sector, sector_size, buffer)
    self.stream:set_position(lshift((cylinder * self.heads + head) * self.sectors + (sector - 1), 9))
    self.stream:read_bytes_to_buffer(sector_size, buffer)
end

local function write_sector(self, cylinder, head, sector, data)
    self.stream:set_position(lshift((cylinder * self.heads + head) * self.sectors + (sector - 1), 9))
