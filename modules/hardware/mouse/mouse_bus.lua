...
local function port_signature_out(self, cpu, port, val)
    self.sig_reg = val
end

local function port_signature_in(self, cpu, port)
    return self.sig_reg
end
...
local function port_config_out(self, cpu, port, val)
...
local function port_config_in(self, cpu, port)
...
local function update_mouse_state(self, delta_x, delta_y, left_pressed, middle_pressed, right_pressed)
...
function mouse.new(cpu, pic, base_addr, irq)
    ...
    cpu_io:set_port_in(addr, port_data_in)
    cpu_io:set_port(addr + 1, port_signature_out, port_signature_in)
    cpu_io:set_port(addr + 2, port_control_out, port_control_in)
    cpu_io:set_port(addr + 3, port_config_out, port_config_in)

    cpu_io:set_in_function_argument(addr, self)
    cpu_io:set_function_argument_range(addr + 1, addr + 3, self)
